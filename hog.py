...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # Free Bacon Rule
    opp_score = [(opponent_score % 10), (opponent_score // 10) % 10]
    if (num_rolls == 0):
        return 1 + max(opp_score[0],opp_score[1])
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    while (score <= 100 or opponent_score <= 100):
        # swapping for current strategy
        cur_strategy = strategy0(score, opponent_score) if (who == 0) else strategy1(score, opponent_score)
        cur_score = score  if (who == 0) else opponent_score
        opp_score = opponent_score if (who == 0) else score

        cur_score += take_turn(cur_strategy, opp_score, dice=select_dice(score,opponent_score))
        # Swine Swap Rule
        if (cur_score * 2 == opp_score):
            cur_score,opp_score = opp_score, cur_score
        
        # updating specific scores of students
        score = cur_score
        opponent_score = opp_score
        
        # this produces error
        #score = cur_score if (who == 0) else opp_score
        #opponent_score = opp_score if (who == 0) else cur_score
...
def make_averaged(fn, num_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    def dice_in(*args):
        valid_dice_in = fn(*args)
        avg = valid_dice_in / len(*args[0])
        return avg
    return dice_in
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max_index = 0
    for i in range(0, 10):
        avg_n_die[i]  = make_averaged(roll_dice, 1000)(10, dice)
        print(i + 1, "dice scores", avg_n_die[i], "on average")
    for i in range(0, 10):
        if  (avg_n_die[max_index] < avg_n_die[i]):
            max_index = i
    max_index += 1 
    return max_index 
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    cur_max = max(score % 10, (score // 10) % 10)
    opp_max = max(opponent_score % 10, (opponent_score // 10) % 10)
    if (cur_max <= opp_max):
...
def swap_strategy(score, opponent_score):
    ...
    opp_max = max(opponent_score % 10, (opponent_score // 10) % 10)
    # BENEFICIAL SWAP
    if (score < opponent_score and score + opp_max == 2 * opponent_score):
        return 0
    # HARMFUL SWAP
    elif (score > opponent_score and score + opp_max == 2 * opponent_score):
        return BASELINE_NUM_ROLLS
    # BASELINE CASES
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***    
    S1. Find a way to leave your opponent with four-sided dice more often.
    S2. If you are in the lead, you might take fewer risks. If you are losing, you might take bigger risks to catch up.
    S3. Vary your rolls based on whether you will be rolling four-sided or six- sided dice. 
    """
    "*** YOUR CODE HERE ***"
    cur_max = max(score % 10, (score // 10) % 10)
    opp_max = max(opponent_score % 10, (opponent_score // 10) % 10)
    # implementing S1
    if (((score + opp_max) + opponent_score) % 7 == 0 and (score + opp_max != 2 * opponent_score)):
        return 0
    # implementing S2
